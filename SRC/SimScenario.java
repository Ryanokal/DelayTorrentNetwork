...
public class SimScenario implements Serializable {
	...
	/**CHANGES ADDITIONAL STRINGS FOR CUSTOM CONFIG SETTINGS*/
	private static final String CUSTOM_NAME = "Lee";
	private static final String BH_NUMBER = "numberOfBlackHoles";
	private static final String BH_ROUTER = "blackHoleRouter";
    private static final String BH_RANDOM = "randomDistribution";
	/** The world instance */
	private World world;
	...
	protected void createHosts() {
		...
		Settings customSettings = new Settings(CUSTOM_NAME);	
		int numberOfBlackHoles = 0;
        int low,high;
        int blackHoles[] = new int[numberOfBlackHoles];
        if(customSettings.contains(BH_RANDOM)){
            boolean randomBlackHoles = customSettings.getBoolean(BH_RANDOM);
            numberOfBlackHoles = customSettings.getInt(BH_NUMBER);
            if(randomBlackHoles){
                low = 0;
                high = 130;
            }else{
            ...
            //count total number of hosts, calculate n required black holes for desired percentage
            /*int totalNumberOfHosts = 0;
            for (int i=1; i<=nrofGroups; i++) {
                Settings s = new Settings(GROUP_NS+i);
                int nrofhosts = s.getInt(NROF_HOSTS_S);
                totalNumberOfHosts += nrofhosts;
            }*/
            
            blackHoles=new int[numberOfBlackHoles];
            ...
            //print black hole nodes to console for debugging
            System.out.println("Creating" + numberOfBlackHoles + " Black Holes at nodes: ");
            for(int i=0;i<numberOfBlackHoles;i++){
                System.out.print(blackHoles[i] + " ,");
            }
        }
		//counts current hostNumber
		int currentHostNumber = -1; //first host should be 0
		//MY CODE CHANGES STOP HERE
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
                ...
                if(customSettings.contains(BH_RANDOM)){
                    currentHostNumber++;
                    //specified routing protocl
                    mRouterProto =
                        (MessageRouter)s.createIntializedObject(ROUTING_PACKAGE +
                        s.getSetting(ROUTER_S));
                    //unless chosen to be a black hole
                    for(int k=0;k<numberOfBlackHoles;k++){
                        if (blackHoles[k] == currentHostNumber){
                            String bhRouter = customSettings.getSetting(BH_ROUTER);
                            //change protocol to be black hole
                            mRouterProto =
                                (MessageRouter)s.createIntializedObject(ROUTING_PACKAGE +
                                bhRouter);
                            isBlackHole = true;
                            k = numberOfBlackHoles;	//break loop
                        }
                    }
                }
				...
				DTNHost host = new DTNHost(this.messageListeners,
						...
						mmProto, mRouterProto, isBlackHole);
				hosts.add(host);
			}
		}
	}
